...
Petrify has exactly one heap allocation per invocation when executing trees (the scores accumulator array). This is required to make the compiled model thread-safe. (Future iterations might have an option where thread safety is not a concern, making a model invocation create 0 Garbage Collection pressure.) This also give Petrify compiled model very predictable and consistent latency, if your application is sensitive to that.
