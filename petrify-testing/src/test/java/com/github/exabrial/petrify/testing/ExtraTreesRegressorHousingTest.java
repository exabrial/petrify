...
/**
 * Uses a scikit-learn ExtraTreesRegressor (Extremely Randomized Trees) exported to ONNX via skl2onnx, trained on California housing
 * data to predict median house value (continuous). 30 estimators, max_depth=6, BRANCH_LEQ node mode, post_transform=NONE,
 * aggregate_function=AVERAGE, 8 input features (MedInc, HouseAge, AveRooms, AveBedrms, Population, AveOccup, Latitude, Longitude).
 * 2,778 total nodes (1,374 branch + 1,404 leaf). R²=0.55. ExtraTrees differs from RandomForest in that split thresholds are chosen
